...
import { searchUsers } from '../services/collaborator';
...
const InviteUser = ({ closeModal, configs: { action, title, type, tags } = {} }) => {
    ...
    const onSubmit = (e) => {
        ...
        searchUsers(searchRef.current?.value).then(data => setUsers(data));
    };
        ...
        <div className="modal-dialog modal-lg m-0" role="document">
            <div className="modal-content">
                <div className="modal-header">
                    ...
                    <button onClick={() => closeModal()} type="button" className="btn btn-danger text-white" data-dismiss="modal"><i className="ri-close-fill m-0" /></button>
                </div>
                                        ...
                                        <li className="mb-3 pb-3 border-bottom">
                                            <div className="iq-search-bar members-search p-0">
                                                <form onSubmit={onSubmit} className="searchbox w-auto">
                                                    <input type="text" className="text search-input bg-grey form-control" placeholder="Type here to search..."
                                                        ref={searchRef} />
                                                    <a className="search-link d-none" type="submit"><i className="ri-search-line" /></a>
                                                </form>
                                            </div>
                                        </li>
